...
# Schema Migrations
- Current Dexie schema version: 2
- Always use `.upgrade()` pattern for schema changes — never reuse a version number
- Each new version must increment (v3, v4, etc.)
- Existing data must be preserved — never drop and recreate tables